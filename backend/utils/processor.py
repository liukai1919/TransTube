...
class VideoProcessor:
    def __init__(self, chunk_duration=300, max_workers=4, progress_callback=None):
        """
        初始化视频处理器
        ...
        self.max_workers = max_workers  # 默认4个并行处理线程
    ...
    def split_video(self, video_path: str) -> List[Dict]:
        ...
        self.total_steps = chunk_count * 3 + 2  # 每个切片3步（分割+转录+翻译）+ 合并字幕 + 合并视频
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    self.update_progress(f"处理切片时出错: {str(e)}", 0)
